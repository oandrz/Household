...
| **Telegram** (Bot API) | The second delivery channel for sign-in and sign-up links, when a bot is configured — and, since 2026-09-08, chat commands (`/spend`, `/balance`…) on the same poller. **One token, one poller:** Telegram gives each update to exactly one `getUpdates` caller, so a dev stack sharing the production token loses every message to the other consumer (`terminated by other getUpdates request`, seen 2026-09-08). Since then there are two bots: the production one, whose token lives only in `deploy/.env`, and `@HearthOinkDevBot`, whose token lives only in the developer's local `.env`. Changing `.env` needs `docker compose up -d api`; `restart` keeps the old environment. Outbound only — the API long-polls `api.telegram.org`; there is no webhook and nothing new faces the internet | $0 — free, with no per-message charge and no domain, DNS record, business verification or legal entity required. That is the whole reason it was chosen over WhatsApp and SMS ([ADR 4](adr/0004-telegram-as-a-second-delivery-channel.md)) | **Telegram sign-in stops. Email sign-in is unaffected, and sessions already issued are unaffected** — no session depends on Telegram once it exists, because Telegram only ever carried the link. `POST /api/v1/auth/telegram/start` answers `404` and the sign-in screen hides the control, so the degradation is a missing button rather than a broken one. **Was not on the box on 2026-09-01**; by 2026-09-08 the local `deploy/.env` copy sets both values and something else polls the token, so this may no longer be true — verify on the box before relying on either reading |
| **Anthropic** (Claude API) | Reads a plain sentence sent to the Telegram bot into an expense or income the person then confirms with `/yes` (`api/internal/adapter/anthropic`, ADR 8 stage 5b). **Optional, and off by default**: only built when `ANTHROPIC_API_KEY` is set. Model `claude-opus-5`, effort low, one short request per free-text message | Per message: roughly 300 input tokens and under 100 output at $5 / $25 per million — well under a cent each. A household logging ten sentences a day is cents a month. **Configured on the owner's machine on 2026-09-08 and refused for lack of credits** (`Your credit balance is too low`), which is what prompted the OpenRouter row below; not on the box | Free text stops (the bot says "commands only"); `/spend` and every slash command keep working; nothing else in the product notices |
| **OpenRouter** (OpenAI-dialect chat API) | The same free-text reading as the Anthropic row, through a gateway to open-weight models (`api/internal/adapter/openrouter`). **Optional, off by default, and exclusive with Anthropic**: built only when `OPENROUTER_API_KEY` and `OPENROUTER_MODEL` are both set. Plain `net/http`, no SDK; one forced tool call per sentence; 30 s timeout | $0 on a model whose id ends `:free` (rate-limited, and the free list changes month to month — hence the model is configuration, not code); otherwise cents per thousand sentences on the cheap paid ones. Configured on the owner's machine on 2026-09-08 with a free model; not on the box | Same as the Anthropic row: free text stops, everything else is untouched. A free model that is withdrawn answers with an error the log names (`openrouter chat.completions: 404 …`), and the fix is a new `OPENROUTER_MODEL` in `.env` |
...
| `ANTHROPIC_API_KEY` | `.env` on the owner's machine (gitignored) since 2026-09-08; not on the box. When it is: `deploy/.env`, mode 600, beside the other secrets. Placeholder lines in both `.env.example` files. **Passed into the dev `api` container by name in `docker-compose.yml`** — production's `env_file` reads `.env` whole, the dev file does not (LEARNING j) |
| `OPENROUTER_API_KEY` (travels with `OPENROUTER_MODEL`, which is not a secret) | Same places and the same compose rule as `ANTHROPIC_API_KEY`; `.env` on the owner's machine since 2026-09-08. `config.Load` refuses a key without a model, a model without a key, and either beside the Anthropic key | Yes — revoke at openrouter.ai/settings/keys and issue a new one. **What it can do if it leaks:** spend whatever credit the OpenRouter account holds (nothing, on free models) until revoked; it grants nothing in Hearth. The adapter never logs it, and its errors carry the status and the provider's message only — no URL, no request id | Yes — revoke in the Anthropic console and issue a new one. **What it can do if it leaks:** spend money on the leaker's requests until revoked; it grants nothing in Hearth. The adapter never logs it and its errors carry no URL |
