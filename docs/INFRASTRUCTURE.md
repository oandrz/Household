...
| **Anthropic** (Claude API) | Reads a plain sentence sent to the Telegram bot into an expense or income the person then confirms with `/yes` (`api/internal/adapter/anthropic`, ADR 8 stage 5b). **Optional, and off by default**: only built when `ANTHROPIC_API_KEY` is set. Model `claude-opus-5`, effort low, one short request per free-text message | Per message: roughly 300 input tokens and under 100 output at $5 / $25 per million — well under a cent each. A household logging ten sentences a day is cents a month. **Not configured anywhere today**, so the current cost is $0 | Free text stops (the bot says "commands only"); `/spend` and every slash command keep working; nothing else in the product notices |
...
| `ANTHROPIC_API_KEY` | Not set anywhere yet. When it is: `deploy/.env` on the box, mode 600, beside the other secrets; `.env` locally (gitignored). Placeholder lines in both `.env.example` files | Yes — revoke in the Anthropic console and issue a new one. **What it can do if it leaks:** spend money on the leaker's requests until revoked; it grants nothing in Hearth. The adapter never logs it and its errors carry no URL |
