...
the adapter's edge — 🟡, not walked against the real bot because production
holds the same token; a dev bot token unblocks it.
...
branch `admin-db-browse`, merged as
[PR #18](https://github.com/oandrz/Household/pull/18), `a44b111`
...
(§4, branch `admin-db-browse`, merged as
[PR #18](https://github.com/oandrz/Household/pull/18), `a44b111`, to its own spec
...
**The build order changed a third time on 2026-09-07: the Family calendar is
out of this release.** The product owner's call, taken while answering "what's
next" — Family was the next slice by dependency and nothing about it was
blocked or unbuildable, so this is scope, not discovery. Its three tracker
rows are now 🚫 (§7), the two deferred ones say what would bring them back,
and `docs/FEATURE_TRACKER.md`'s legend grew a third meaning for 🚫 in the same
edit, because every previous 🚫 in that file meant *permanent* and a reader
would otherwise have concluded the calendar is dead. The dark-shipped
`GET /api/v1/family/calendar` stub stays exactly where it is: it belongs to
the flags spec's dark-shipping proof, not to the calendar. The recount is in
the tracker's own log — **86/18/15/5 = 124**, Built + Partial **104**, with
**119** the denominator that means anything for this release.

**"This release" is not defined in any document here** and never has been —
the product ships continuously to one box. It is read as the first sold
version. One line in the tracker's 2026-09-07 entry corrects that if the
owner means a different boundary.

**So the next work is not a slice.** With Family out, no remaining ⬜ row is
one; what is left across §§1–6 is eighteen 🟡 gaps and fifteen ⬜ rows that do
not share a shape. `docs/FEATURE_TRACKER.md`'s "Suggested order" now
recommends the one group with a single root cause: **Notifications delivery**
— four 🟡 rows (bill due, overspend, retro reminder, weekly digest), each
stored, served, editable, and never sent. It is the only place in the product
that currently tells a household something untrue: the design's copy promises
delivery and the switch delivers nothing. Two existing decisions it must
contradict *explicitly* rather than quietly — nothing here runs on a clock
(Budget decision 1 and Goals decision 4 refused this product's first scheduler
outright; Bills decision 3 records the consequence in the product's own copy),
and the Bills spec's own "Out of scope" section already calls bill reminders,
automatic goal contributions and automatic month-end rollover **"one missing
piece, not three gaps"** — so the work also closes §5's "Roll unspent into
savings", making it five 🟡 rows across two sections behind one cause. And
[ADR 3](adr/0003-mail-stays-on-the-box.md) means mail
reaches nobody in production, so Telegram ([ADR 4](adr/0004-telegram-as-a-second-delivery-channel.md))
is the channel and a household with no bot linked is the case to design for.
It is also the moment ADR 4's deferred `Notifier` port earns its existence on
ADR 4's own terms: a scheduled reminder is the second *caller*, the first code
here that must choose a channel.
...
question that deserves its own conversation"). **Agreements had to settle one
question before its spec could be written: what "both sign" means in a
household with one owner. Settled 2026-09-05 by the product owner — a
household with fewer than two owners does not see Agreements at all.** Of the
three answers put to them (a single owner's own signature completes a
proposal; proposals pend until a second owner exists; hide the section until
there are two) the third was chosen, so "both sign" means two and never one.
...
and the "To discuss" block on Retros) are built and reviewed clean, and then
**walked: Agreements' own fifteen-criterion browser walk ran 2026-09-06 and
passed 15 of 15** (`docs/superpowers/plans/2026-09-05-hearth-agreements-verification.md`),
followed by the whole-branch review that ran against the resulting branch
head (`f1631a4` on `agreements-spec`, since merged to `main` as `a969f39`, PR #20) and this fix wave's
own commits closing its findings. `docs/FEATURE_TRACKER.md`'s Agreements
rows carry the locked state's both halves, the propose → sign lifecycle, and
two rows the design never drew — parking a proposal for the next retro,
restoring a removed one from history — and **eight of them moved 🟡 → ✅ when
the walk passed**, the same bar every other feature in this file was held to.
**Marriage (slice 3) is done**: Retros, Vision and Agreements are all
code-complete, reviewed, and each walked 15/15 in a real browser — the walk
also found and fixed one real product defect, a `previousBody` read live
instead of snapshotted in `ProposeAgreementModal.tsx` (`1325197`; the table
above has the full account). Nothing in it needs more design or a further
conversation with the product owner — the two rows still ⬜ (Vision's
marriage-duration row, the "Popular starting points" cards) are permanent
decisions, not open work. `docs/FEATURE_TRACKER.md`'s "Suggested order"
reflects this: item 6 needs no further build, and ~~item 7, Family, is where
the next spec opens~~ — **superseded 2026-09-07, when the product owner
deferred the Family calendar out of this release.** Item 7 is struck through
there, §7's three rows are 🚫, and the next spec opens on Notifications
delivery instead. See "What to do next" above.

With Marriage closed, the production deployment remains the other
