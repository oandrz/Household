...
sequenceDiagram
    ...
    alt caller's role is not owner
        ...
        Note over Svc,Conv: one Converter per request
        loop each counted account
            Svc->>Conv: Convert(balance)
            Conv->>FX: Rate(account currency, primary) unless already<br/>primary, and only the first time per currency
            Conv-->>Svc: amount in primary, or an error
            Svc->>Svc: ErrNoRate: exclude and name the account<br/>any other error: fail the request<br/>else Add — domain.Money.Add refuses two currencies
        end
...
**Only "no rate" leaves an account out; every other conversion failure fails
the request.** `usecase.Converter` is shared by every screen that adds money
...
outage would have shown a smaller total with a false reason beside it
(`docs/LEARNING.md`, pattern 5). The Converter also remembers each rate for the
request, so two figures on one screen can never be converted at two rates. A
single-currency household never asks the provider at all, so it keeps working
during an outage.
...
sequenceDiagram
    ...
    Note over Svc,Conv: one Converter per request
    loop each expense
        Svc->>Conv: Convert(amount)
        Conv->>FX: Rate(transaction currency, primary) unless already<br/>primary, and only the first time per currency
        Conv-->>Svc: amount in primary, or an error
        Svc->>Svc: ErrNoRate: exclude, list in ExcludedNoRate<br/>any other error: fail the request<br/>else Add
    end
...
**An expense is left out of Spent only when its currency has no rate**
(`domain.ErrNoRate`). Any other conversion error fails the request, by the
