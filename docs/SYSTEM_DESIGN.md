...
graph TD
    ...
    ANT["api.anthropic.com<br/>outbound only, only when ANTHROPIC_API_KEY is set;<br/>one short request per free-text chat message"]
    ORT["openrouter.ai<br/>outbound only, only when OPENROUTER_API_KEY<br/>and OPENROUTER_MODEL are set; the same one<br/>request, to an open-weight model instead"]
    ...
    API -->|"HTTPS: POST /v1/messages"| ANT
...
graph TD
    ...
    subgraph adapters["internal/adapter/ — implements the ports"]
        ...
        AnthropicA["anthropic — IntentParser over the<br/>Claude API: one strict tool call reads a<br/>sentence into an intent. Built only when<br/>ANTHROPIC_API_KEY is set; never writes"]
        OpenRouterA["openrouter — the same IntentParser<br/>over OpenRouter's OpenAI-dialect chat API,<br/>plain net/http, forced tool call. Built only<br/>when OPENROUTER_API_KEY + _MODEL are set"]
        IntentShared["intent — what both parsers share:<br/>the prompt, the tool schema, and the<br/>fail-closed reader of its arguments"]
        Clock["clock"]
        FX["fx — static rates"]
    end
...
| `IntentParser` | `adapter/anthropic`, `adapter/openrouter` | The Telegram bot's free-text reader (stage 5b of the chat-commands spec). Two implementations, chosen by configuration and never both (`config.Load` refuses that): Claude through the Anthropic SDK, or an open-weight model through OpenRouter's OpenAI-dialect API over plain `net/http`. What they share — the prompt, the `log_transaction` schema, and the reader that turns the model's arguments into an `Intent` and fails closed on any kind it did not name — lives once in `adapter/intent`, because the reader is the last line between a model's output and the ledger and must be identical whichever model spoke. `nil` means "commands only". The port returns text fields, never ids, so what the person confirms is what they can read. The Commander caps every call at 30 s, because the poller handles one update at a time and a stalled provider would hold every chat |
...
asks `usecase.IntentParser` (the `anthropic` or the `openrouter` adapter,
whichever the configuration names, under a 30 s deadline) to read it,
...
| Intent parsing | **Off unless configured.** `ANTHROPIC_API_KEY` set means the Telegram `Commander` gets an `IntentParser` over Claude (`adapter/anthropic`, `claude-opus-5`, effort low, one strict tool call, `max_tokens` 4096); `OPENROUTER_API_KEY` **and** `OPENROUTER_MODEL` set (both or neither, and never beside the Anthropic key — `config.Load` refuses either mistake) means the same parser over an open-weight model through OpenRouter (`adapter/openrouter`, forced tool call, `max_tokens` 4096, 30 s client timeout; the model id is configuration because OpenRouter's free, tool-capable list changes month to month). Neither set means a plain sentence to the bot from an authorised owner is answered "commands only" and a stranger's is ignored. The start-up log line names `provider` and `model`. No key is ever logged and neither adapter's errors carry a URL, a request id or a workspace id. A parse is never a write: the reading is shown back and held five minutes per chat in the poller's memory, written on `/yes` with the sentence's update id as the idempotency key. Not run against the real API on this machine (no key) — tested against a fake Messages API |
