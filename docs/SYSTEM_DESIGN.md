...
(`docs/LEARNING.md`, pattern 5). The Converter also remembers each answer (a
rate, or "no rate") for as long as it lives, so every amount it converts in one
currency gets the same answer. That guarantee is per Converter, not per page:
a service that builds two, or a page that calls two endpoints, can ask the
provider more than once. A
