...
| `IntentParser` | `adapter/anthropic` | The Telegram bot's free-text reader (stage 5b of the chat-commands spec). One implementation and one caller, which is normally the wrong shape for a port — it is one anyway because the implementation is a paid third-party API behind a key, and the product must behave identically without it: `nil` means "commands only". The port returns text fields, never ids, so what the person confirms is what they can read |
...
asks `usecase.IntentParser` (the `anthropic` adapter, only when a key is
configured) to read it, shows the reading back, and writes it only on
...
| Intent parsing | **Off unless configured.** `ANTHROPIC_API_KEY` set means the Telegram `Commander` gets an `IntentParser` (`adapter/anthropic`, `claude-opus-5`, effort low, one strict tool call, `max_tokens` 4096); unset means a plain sentence to the bot from an authorised owner is answered "commands only" and a stranger's is ignored. A single value with no pairing rule. The key is never logged and the adapter's errors carry no URL. A parse is never a write: the reading is shown back and held five minutes per chat in the poller's memory, written on `/yes` with the sentence's update id as the idempotency key. Not run against the real API on this machine (no key) — tested against a fake Messages API |
