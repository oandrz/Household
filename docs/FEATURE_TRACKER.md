...
with two caveats (see that row). **A second ✅ below cites no walk at all,
yet**: "Outbound message inspector", built 2026-09-04, is code-complete and
has not been walked in a browser — its own cell says so plainly rather than
implying the bar every other ✅ here meets.
...
3. ~~**Outbound message inspector**~~ (§5) — **built 2026-09-04**, to its own
   ...
   HTTP API, audited. **Code-complete, not yet walked** — the fifteen-criterion
   browser walk is Task 9 of the plan and has not run; section 9's row names
   the verification file it will carry once it does. Numbering kept, as for
   items 1 and 2.
