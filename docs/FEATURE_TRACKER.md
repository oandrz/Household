...
four, and 🟡 rather than ✅ for the two reasons its own cell names. **The
operator surface is complete, and "Suggested order" below has moved on to the
household product.**
...
4. ~~**Read-only database browse**~~ (§4) — **built 2026-09-04**, to its own
   ...
   🟡, not ✅**, for two reasons its own cell names: the fifteen-criterion
   browser walk has not run, and production ships with the feature switched
   off by the owner's decision. Numbering kept, as for items 1–3.
