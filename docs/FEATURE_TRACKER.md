...
> not draw are added to §1, all ✅: per-IP limits on sign-in and the
> magic-link request, the login-CSRF refusal, and the browser security
> headers. Entry & authentication goes 14/2/1/0 → **17/2/1/0**, recounted by
> symbol over §1's twenty rows. Every other section recounted the same way and
> unchanged (§9 reads 7/1/0/1 by eye; a first scripted count said 6 and was
> wrong, not the table). Totals 112/20/14/5 → **115/20/14/5 = 154**, Built +
> Partial **135**, denominator **149**.

> **Recounted 2026-09-23 (browser walk)**, when Task 9 of the same
> milestone walked the household access list in a real browser, 14 of 14
> (`docs/superpowers/plans/2026-09-23-hearth-household-access-list-m3-verification.md`).
> Two section-10 rows move 🟡 → ✅: "Manage API tokens in Settings" and
> "Household access list — every member's tokens and chats (no mockup)".
> Section 10 goes 8/2/0/0 → **10/0/0/0**; totals 110/22/14/5 →
> **112/20/14/5 = 151** (Built +2, Partial −2). Built + Partial stays
> **132** (110+22 = 112+20), denominator stays **146** (151 − 5). **The
> summary table below had been left at the 2026-09-20 values** (Automation
> 8/0/1/0, total 110/20/15/5 = 150) when the recount under this one was
> written; it now matches a fresh count of the State column, split on
> unescaped pipes only.
>
> **Recounted 2026-09-23**, when Task 8 of the partner-invite lobby's
> milestone 3 (the household access list) landed its docs. Section 10 gains
> one new row for a capability the design never drew at all, "Household
> access list — every member's tokens and chats (no mockup)" — `GET
> /api/v1/household/access`, `docs/superpowers/specs/2026-09-23-hearth-household-access-list-design.md`
> — and "Manage API tokens in Settings" moves ⬜ → 🟡 now that the Access
> panel actually ships the token list, Revoke and the New token dialog. Both
> new/changed rows carry the same caveat: **built, not yet walked** — Task 9
> of the same milestone does that walk and is expected to flip them to ✅.
> Counted with the 2026-09-20 method (State column, split on unescaped pipes
> only). New section 10 total 8/2/0/0 (from 8/0/1/0: one row converted
> ⬜ → 🟡, one row added as 🟡). Every other section unchanged. New totals:
> 14/2/1/0, 7/1/1/0, 14/8/2/0, 8/2/1/0, 36/4/7/0, 16/2/2/0, 0/0/0/3, 0/0/0/1,
> 7/1/0/1, **8/2/0/0** — **110/22/14/5 = 151**, Built + Partial **132**,
> denominator **146**.
>
> **Recounted 2026-09-20**, when the partner-invite lobby's milestone 2 landed
> (`docs/adr/0011-joining-a-household-by-knock.md`). "Telegram invites — a
> shareable `t.me/…?start=inv_<token>` link" (section 1) moves ⬜ → ✅, its
> justification text replaced rather than re-flagged: the link, the knock and
> Let in are real now, not deferred. Three rows the design does not draw join
> the map under this file's own rule for work the design does not describe:
> "The bot answers only private chats" (section 1, closing security-review
> finding 2 — `docs/reviews/2026-09-19-security-review.md`), "Email invites
> behind a flag, off by default" and "The invite lobby: link in hand, a
> knock, Let in or Not them" (both section 3). "Invite a family member
> (modal)" (section 3) stays ✅ with its note rewritten: the modal now picks a
> channel rather than taking a bare optional email. Counted with the
> 2026-09-19 method (State column, split on unescaped pipes only, a row
> detached from its table by a blank line still reads that table's State
> column) — the script reproduced the previous totals exactly, 106/20/16/5 =
> 147, before these edits. New section 1 total 14/2/1/0 (from 12/2/2/0: two
> rows turned ✅, one of them a flip from ⬜, one newly added), new section 3
> total 14/8/2/0 (from 12/8/2/0: two rows added, both ✅). Every other section
> unchanged. New totals: 14/2/1/0, 7/1/1/0, **14/8/2/0**, 8/2/1/0, 36/4/7/0,
> 16/2/2/0, 0/0/0/3, 0/0/0/1, 7/1/0/1, 8/0/1/0 — **110/20/15/5 = 150**, Built
> + Partial **130**, denominator **145**. Three of the four new/changed ✅ rows
> (Telegram invites, the invite lobby, and email invites' UI half) say plainly
> that milestone 2's own browser walk (Task 15) has not run yet, the same
> exception to the legend this file already carries for the outbound message
> inspector's first day. The fourth, "the bot answers only private chats,"
> does not carry that caveat: its coverage is test-level
> (`chat_type_test.go`), because exercising the refusal in a browser walk
> would need a second bot added to a Telegram group, outside the scope of the
> usual owner-signed-in walk.
>
...
>
> **The read-only database browse, 2026-09-04 — code-complete and reviewed;
> browser walk not run, and switched off in production on purpose.**
> Section 9's last ⬜ moves ⬜ → **🟡**, and the section now has none. It is
> the fourth and last of the four platform-administration features named on
> 2026-09-02, so **the operator surface is complete** and "Suggested order"
> below moves on to the household product. What shipped: a second connection
> pool as a `SELECT`-only Postgres role, a `DatabaseBrowser` port with two
> implementations, two granted routes and one screen at `/admin/database`.
> Spec:
> `docs/superpowers/specs/2026-09-04-hearth-database-browse-design.md`.
>
> **Why 🟡 and not ✅, when the outbound message inspector above took ✅ under
> the same missing walk.** That row stated its exception plainly and
> earned it; this one has a *second* gap that row did not, and the two
> together are more than a caveat. Production ships with
> `DATABASE_READONLY_URL` unset, by the product owner's decision on
> 2026-09-04 — so this feature is built everywhere and running nowhere real
> until an operator runs `deploy/PROVISION.md` section 10. Combined with a
> walk that has not happened, on the one surface in this product that can
> read every household's finances, the conservative symbol is the honest
> one. The row names both gaps and says what closes each. It becomes ✅ when
> the walk passes and its record is cited there; whether production is
> switched on is the owner's call and does not gate the symbol.
>
> **The recount is a count, not an addition**, by this file's own rule — the
> first status symbol in each row's own State cell, over every section, not a
> delta applied to the previous table. Section 9 gives **6 ✅ / 2 🟡 / 0 ⬜ /
> 1 🚫** across its nine rows. Only section 9 changed, and the Total row is
> re-summed from all nine section rows as they now stand:
> **79/18/20/3 = 120**, Built + Partial **97**. The ✅ column does not move,
> because no row became verified today; a ⬜ became a named 🟡, which is the
> whole difference.
>
> **The read-only database browse's browser walk ran 2026-09-04, later the
> same day — Task 11 of the same plan
> (`docs/superpowers/plans/2026-09-04-hearth-database-browse-verification.md`):
> 15 of 15 criteria pass.** Section 9's "Read-only database browse" row moves
> 🟡 → ✅. That row carried a 🟡 for exactly one reason the walk could close —
> the walk had not run — and the second gap it named, production shipping with
> `DATABASE_READONLY_URL` unset, was always the owner's call and explicitly
> did not gate the symbol. Both 503 states were driven, the write refusal was
> run against the live `hearth_readonly` role (both guards proven
> independently — `default_transaction_read_only` refuses first, and `SET
> TRANSACTION READ WRITE` gets past that only to meet `permission denied for
> table households`), the audit delta was exactly one row naming the table and
> the offset, and all four viewport widths were measured on both screens —
> none of which had ever rendered in a browser, because task 9 could not: no
> vite service was running. **One defect found and fixed in the walk, and it
> was prose rather than behaviour** — two comments explained `«null»` with
...
> **7 ✅ / 1 🟡 / 0 ⬜ / 1 🚫**. Only section 9 changed, and the Total row is
> re-summed from all nine section rows as they now stand, not adjusted by
> delta: **80/17/20/3 = 120**, Built + Partial **97** — the headline does not
...
> **Agreements, 2026-09-05 — code-complete and reviewed across eighteen
> tasks (`915edba`).** Section 6's five ⬜ rows named in the design
> (Agreements by section, its empty state, Propose, New section, Version
> history) and the locked-state row added 2026-09-05 all moved ⬜ → 🟡, and
> two rows the design never drew joined them as 🟡 — parking a proposal for
> the next retro, and restoring a removed one from version history — the
> same treatment Goals' contributions and Accounts' archive/restore rows
> already have. A ninth row, "'Popular starting points' — tapping a card,"
> was added as ⬜: the design draws the four cards but gives them no
> `onClick`, and every agreement has to arrive through propose → sign, so
> this is drawn-but-not-built like the ⌘K chip and the "45 min" retro
> duration, not a gap.
>
> **Corrected here, not appended to — the tracker's own rule for a symbol
...
> Partial **105**, unchanged from the moment before this walk ran, since
> moving eight rows from 🟡 to ✅ moves neither the denominator nor Built +
> Partial, only which of the two columns each sits in. "105 of 124" reads
...
**125**. Then free text through Claude, also 🟡 (no key here): **90/20/16/5 =
131**, denominator **126**. Then, once a development bot existed, chat
commands were walked live and moved 🟡 → ✅: **91/19/16/5 = 131**,
denominator **126** (unchanged). Then free text, rebuilt on OpenRouter and
walked live, 🟡 → ✅: **92/18/16/5 = 131**, denominator **126**. Then the
...
| Magic link — consume and sign in | ✅ | Works while the household is locked; that is the recovery path. Since 2026-09-25 opening the link shows "Continue signing in" and consumes the token only on that click, with a "Signed in as …" warning when this browser already has a session — see the login-CSRF row below. Walked 2026-09-25: link opened while signed in showed the warning and waited; the click landed on Overview |
...
| Sign-in and magic-link request limited per client address (no mockup) | ✅ | Security review 2026-09-25, H1. Every sign-in runs a 64 MiB argon2id derivation, including the decoy for unknown addresses, and the route had no limit, so ~60 parallel requests could exhaust the 4 GB production box. Now: 20 sign-ins per 15 minutes and 10 magic-link requests per hour per IP (separate buckets, `router.go`); at most one argon2 derivation per CPU at once, the rest waiting (`crypto.Argon2Hasher`); and a `mem_limit` on every production container. Tests: `TestSignInPassesThroughThePerIPLimiter`, `TestMagicLinkRequestPassesThroughThePerIPLimiter`, `TestArgon2DerivationsAreBoundedInConcurrency`, each mutation-checked. Checked live 2026-09-25: repeated sign-ins against the running stack answered 429 once the bucket was spent |
| Login CSRF refused — another site cannot sign a visitor in (no mockup) | ✅ | Security review 2026-09-25, M1 (first reported 2026-09-19). Every JSON body must now be declared `application/json` (`415 UNSUPPORTED_MEDIA_TYPE` otherwise, `errors.go`), which an HTML form on another site cannot send; and the magic-link screen waits for a click (row above). Tests: `TestSignInRefusesABodyThatIsNotDeclaredJSON`, `MagicLinkConsumeScreen.test.tsx`, both mutation-checked. Walked 2026-09-25: a real auto-submitting `text/plain` form on `localhost` posting to the app on `127.0.0.1` was answered 415 with no cookie set |
| Browser security headers — HSTS, CSP, frame blocking (no mockup) | ✅ | Security review 2026-09-25, M2. `web/nginx.conf` now sends `Strict-Transport-Security`, a strict `Content-Security-Policy` (no inline script, no eval, `frame-ancestors 'none'`), `X-Frame-Options`, `nosniff`, `Referrer-Policy: no-referrer` and `Permissions-Policy`, and hides its version. Pinned by `web/src/nginxSecurityHeaders.test.ts`. Walked 2026-09-25 on the production nginx image: nine pages loaded with no CSP violation, an injected inline script was blocked, and the invite QR SVG rendered. **Not yet live on oink.mywire.org** until the next deploy |
...
| Telegram sign-in — a returning member | ✅ | The bot resolves the chat to its bound user and sends the **existing** magic link, 15 minutes, single use — no new token type and no new session-issuing code anywhere. Three limits stack: 20/hour per IP on the start route (its own bucket, not sign-up's), 3 links/hour per chat, and the shared global daily sign-up ceiling. An unknown, expired, consumed, rate-limited or ceiling-blocked `/start` all get one identical refusal, word for word, so none of them can be told apart by probing. **Walked against the same real bot and passed, 2026-09-01**: a bound chat's `/start` sent a sign-in link rather than a second sign-up link — the discriminating result, since an unbound chat would have minted a second signup and could have created a second household, and the household count held at 6 throughout — tapping it signed the same user in (since 2026-09-25 the tap opens a "Continue signing in" page and the sign-in happens on that click, like every magic link — see the login-CSRF row), and the spent link and the fourth `/start` within the per-chat window were both refused with the identical wording. Full record: `docs/superpowers/plans/2026-09-01-telegram-sign-in-verification.md` |
| The bot answers only private chats | ✅ | Partner-invite lobby milestone 2, 2026-09-20, closing security-review finding 2 (`docs/reviews/2026-09-19-security-review.md`). A bound **group** chat used to make every member of the room the account holder: any member could run `/spend`, confirm another member's pending transaction, and a working sign-in link would be posted into the group. `Message.Chat` now carries Telegram's own `Type`, and `isPrivateChatWithItsOwner` (`adapter/telegram/update.go`) gates both `ParseStart` and `ParseCommand` on a `switch` whose `default` refuses (fail closed on a value this build did not construct, per `CLAUDE.md`) plus `From.ID == Chat.ID`, the arithmetic that only holds one-to-one. Covered by `chat_type_test.go` and updated `update_test.go`/`commands_test.go` cases — this row's own ✅ rests on that test coverage rather than a browser walk, since a walk would need a second bot added to a group chat to exercise the refusal at all, outside the scope of the usual owner-signed-in walk. **A second gate, `CHECK (chat_id > 0)` on `telegram_accounts` and `telegram_link_requests`, is deliberately not added this milestone** — those two tables hold real production rows and the ALTER needs a restored-dump run first (migration `00011`'s own rule), plus confirming against the Bot API that private chat ids are always positive. `invites.knock_chat_id` gets that CHECK for free (`invites_knock_chat_is_a_person`), because no row has ever had the column. The database-level gate on the two older tables stays a live, un-closed security-review item, not silently dropped |
| Telegram invites — a shareable `t.me/…?start=inv_<token>` link | ✅ | Partner-invite lobby milestone 2, 2026-09-20 (`docs/adr/0011-joining-a-household-by-knock.md`, closing [ADR 4](adr/0004-telegram-as-a-second-delivery-channel.md)'s "Invites over Telegram" out-of-scope item). An owner creates a Telegram-channel invite and gets a one-time `t.me/<bot>?start=inv_<token>` link (copy, QR, share), shown once. The partner's tap records a **knock** — one per link, drawn from `crypto/rand` as four display-only digits shown in both the partner's chat and the owner's browser — and puts nobody in the household by itself. The owner compares the digits by eye and clicks **Let in** (one transaction: user, membership, `telegram_accounts` row and acceptance stamp, sign-in link sent after the commit) or **Not them** (kills the link, issues a fresh one, tells the dead chat). Migration `00021_invite_channels.sql` adds the channel and four knock columns to `invites`, guarded by four CHECK constraints. See `docs/SYSTEM_DESIGN.md` §5 for the flow and §6 for the table. Not yet walked in a real browser — that is milestone 2's own Task 15, not run as of this doc update |
...
| Invite a family member (modal) | ✅ | Name, role, access switches, and — since the partner-invite lobby's milestone 2 — a **channel** choice (Telegram link, email, or profile-only for a kid) instead of a bare optional email field. `availableChannelsFor` (`InviteMemberModal.tsx`) offers only what the household's flags actually allow: Telegram when `telegram_sign_in` is on, email when `email_invites` is on (see the flag's own row below), and profile-only always, since a kid's no-sign-in path writes no invite row and needs no flag. Picking Telegram submits no address at all; the modal then holds the 201 response's one-time link and hands off into the same waiting-card state the pending-invite list uses (see below), so the link is never shown a second time |
| Email invites behind a flag, off by default | ✅ | Partner-invite lobby milestone 2, 2026-09-20. New flag `email_invites` (`domain.FlagEmailInvites`), default **off**, enforced at the HTTP edge (`POST /household/members/invite` answers `409` with `EMAIL_INVITES_DISABLED` for an email-channel request while it is off) and in the UI (the invite modal never offers the option). Off because production mail cannot leave the box ([ADR 3](adr/0003-mail-stays-on-the-box.md)): an email invite would land in the operator's own Mailpit and the partner would never receive it, the same reasoning `notification_delivery` already follows — a flag that is on for something that cannot happen is a lie. The operator turns it on the day a real relay exists, with no code change. See [ADR 11](adr/0011-joining-a-household-by-knock.md). The UI half (the modal hiding the option) is not yet walked in a real browser, the same exception as the two rows above |
| Pending invites in Settings: list and withdraw (no mockup) | ✅ | Partner-invite lobby, milestone 1 (`docs/superpowers/plans/2026-09-19-hearth-partner-invite-lobby-m1.md`), walked 15 of 15 in a real browser on 2026-09-19 (`docs/superpowers/plans/2026-09-19-hearth-partner-invite-lobby-m1-verification.md`). Owners see a **Pending invites** list under Members: each invite nobody has accepted and that has not expired, as `Owner · email · Expires <date>`, with Withdraw. `GET /api/v1/household/invites` is owner-only, and a personal API token may read it. **Withdraw is `DELETE /api/v1/household/invites/{id}`: it needs a browser session (a token gets `403 SESSION_REQUIRED`) and it deletes the row**, so the emailed link then shows the invite-not-found screen. An accepted invite cannot be withdrawn (`409`). A failed withdraw (`404` or `409`) still refreshes the list, and a `409` also refreshes Members. A limited member sees no list and sends no request. At phone width only the address truncates, so the role and the expiry stay visible. **"+ Invite" opens the invite modal on Kid (the design's default); the two "Invite your partner" links (`/settings?invite=partner`, from Overview and Agreements) open it on Parent** — the final-review fix wave, re-walked 2026-09-19. Built on branch `partner-invite-lobby`; not merged or deployed yet. Milestone 2 (below) is what actually gets a partner in |
| The invite lobby: link in hand, a knock, Let in or Not them | ✅ | Partner-invite lobby milestone 2, 2026-09-20. The row above only lists an invite; this is what turns a Telegram invite into a member. Each pending row carries its `channel` and, once tapped, its `knock` (who tapped, as Telegram identifies them, and the four-digit code). `PendingInviteCard` renders the state the row itself carries — link in hand (Copy / QR / Share via `InviteLinkShare`), knocked (compare the code by eye, **Let in** or **Not them**), or admitted (a notice that outlives the row, since Admit's own refetch removes the row from the list before the mutation's `.data` would otherwise arrive — see `docs/LEARNING.md`). `usePendingInvites` polls `GET /household/invites` every 3 seconds **only while at least one Telegram invite has no knock yet**, not on every render of the list. **Get a new link** and **Not them** are the same route (`POST /household/invites/{id}/link`): both kill every earlier copy and clear any knock. **Let in** (`POST /household/invites/{id}/admit`) reports `signInSent: false`, named rather than hidden, when the member was created but the bot could not reach their chat. Not yet walked in a real browser — milestone 2's own Task 15, not run as of this doc update |
...
| Start retro (modal) with mood, money check-in and actions | 🟡 *(the design's own "45 min" duration is drawn, not built — spec decision 8. **Gap found 2026-09-06, during Agreements' Task 18 walk, sweeping for a sibling of that walk's own `previousBody` live-read defect**: `useRetro.ts`'s `saveMutation` reads `query.data.retro.version` live at send time (lines 144-161), while `RetroModal.tsx` seeds its own mood/wentWell/wasHard/notes draft once and never re-seeds it. `addAction`, callable from inside the still-open modal (lines 242, 281), succeeds and invalidates the retro query, forcing a refetch that pulls in whatever version a partner's own concurrent write already committed, landing under the untouched draft — the next Save then sends stale text with a version that now matches, a silent last-write-wins with no error either side can see (window focus after `staleTime`'s 30 seconds is a second, wider trigger for the identical refetch — `main.tsx:11`). Not the same route Retros Task 13 already fixed (that closed the post-conflict Reload button only); this route never reaches a conflict at all. **Not fixed here** — pre-existing code in an already-shipped feature, out of scope for an Agreements branch. The open question is narrower than "needs its own product decision" implies: the product already answers concurrent edits with `RETRO_CHANGED`/`VISION_CHANGED` and a one-way `hadConflict` latch, and snapshotting the version at modal-open (the same shape as this branch's own `1325197`) is what makes that existing answer fire on the first stale Save. **What still needs deciding**: what a *second* Save from the same still-open modal should do after the *first* one already succeeded — refresh the snapshot from that write's own response, or leave it as first taken. Drafted, not filed, at `docs/superpowers/plans/2026-09-06-retro-vision-stale-version-issue.md`. `docs/LEARNING.md` pattern 18 and pattern 1's newest bullet)* |
...
| Park a proposal for the next retro (no mockup — see below) | ✅ *(Discuss moves a proposal to `parked`, where it stays open and answerable, and it appears in a read-only "To discuss" block on the Retros page — decision 7. **No writes into retro tables and no foreign key to a retro row**: the next retro usually does not exist yet, and the block is frontend composition off the same `GET /marriage/agreements` document, sharing one query key so agreeing in either place refreshes both. The block lists parked proposals only, and offers Agree but neither Discuss nor Withdraw — a reminder of what the retro should cover, not a second editing surface. Code-complete and reviewed; **the feature's own fifteen-criterion browser walk (Task 18) has now run and passed, 15 of 15** (2026-09-06) — criteria 8 and 9 walked this live, together, on a household that genuinely had never started a retro yet, with a real park note typed rather than left blank, and confirmed both Agreements and Retros reflect an Agree with no manual reload, in `docs/superpowers/plans/2026-09-05-hearth-agreements-verification.md`)* |
| Restore a removed agreement from version history (no mockup — see below) | ✅ *(the history entry for a removal offers Restore, which opens Propose in add mode pre-filled with the removed wording — an ordinary proposal every owner still has to agree, decision 18. Anything else would let one owner put back alone what two owners agreed to take out. It works because nothing is ever deleted: a removal is a stamp on the row, decision 9. Code-complete and reviewed; **the feature's own fifteen-criterion browser walk (Task 18) has now run and passed, 15 of 15** (2026-09-06) — criterion 11 proposed a remove, agreed it, opened Version history, and clicked Restore: exactly one dialog was open at any time, seeded correctly, focus moved inside it, and the now-empty section was genuinely offered by the picker, in `docs/superpowers/plans/2026-09-05-hearth-agreements-verification.md`)* |
...
both of their halves, where the design has more than one to name. **The
...
**There is no ⬜ left in this section.** The four features that were
deliberately out of scope for the slice that built the rest have all been
settled: the audit screen was descoped by the owner on 2026-09-02, households
and metrics was built the same day, the outbound message inspector on
2026-09-04, and the read-only database browse the same day — the last of the
four, and ✅ since its own fifteen-criterion browser walk ran later that same
day and passed 15 of 15. **The operator surface is complete and verified, and
"Suggested order" below has moved on to the household product.** One 🟡
remains in this section, and it is the flags screen, not the browse.
...
| Admin audit screen (`/admin/audit`) | 🚫 | **Descoped by the product owner on 2026-09-02 — not the design's marking, the owner's decision.** It was built first: `GET /admin/audit?limit=N` in the granted group, `RecentAdminAudit` joined to `users` so rows named their actor, an `AdminAuditPage` with limit-only "Show more" to the service's 500 cap, and a Flags · Audit nav in `AdminShell`; every suite green, three mutation checks red, and a browser walk that found and fixed one defect (the active nav link was indistinguishable — see `docs/LEARNING.md`, Frontend). The owner then judged the screen unnecessary and asked for it to be removed rather than merged. The code is gone from the tree; the log is read through `psql` as before. `AdminService.RecentAudit`, kept at the time for the tests that used it, was deleted on 2026-09-13 together with `AdminAuditRepository.Recent` and its query: nothing in production reached any of it, and a test that exists only to test unreachable code protects nothing (`docs/reviews/2026-09-13-codebase-review.md` §4.2). The one thing kept is unrelated to the screen: `useAdminFlags` no longer refetches on window focus, because every refetch of an audited route is itself an audit row. A patch of the removed work was saved outside the repository at the time, but the honest statement is that reinstating it means rebuilding from the spec (§2.4, §7) |
| Read-only database browse | ✅ | Design spec §4, expanded in `docs/superpowers/specs/2026-09-04-hearth-database-browse-design.md` (which wins where the two differ). **The piece originally asked for, and the last of the four.** **Walked 2026-09-04, Task 11 of the plan — 15 of 15 criteria pass** (`docs/superpowers/plans/2026-09-04-hearth-database-browse-verification.md`), which is what moved this row from 🟡 to ✅: the row carried a 🟡 for the sole reason that the walk had not run. Everything was exercised against the real stack — both 503 states, the write refusal against the live `hearth_readonly` role, the audit delta, and all four viewport widths on both screens, none of which had ever rendered in a browser before (task 9 could not; nothing was serving the frontend). **One defect found and fixed in the walk, and it was prose, not behaviour:** the comments on `domain.NullCell` and on the screen's `Legend()` both explained `«null»` with `users.password_hash`, which is indeed NULL for a magic-link-only member — and renders `«redacted»`, never `«null»`, because redaction is substituted into the `SELECT` list unconditionally and wins over NULL. The one example offered for what the marker is *for* was the one column where it cannot appear. Both now name `users.email` and `goal_contributions` and warn the reader off `password_hash`; no new test, because the corrected claim was already asserted by `browse_repo_test.go:142`, which was mutation-checked instead (it went red with `a NULL in a redacted column rendered as "«null»", want "«redacted»"`). The uncomfortable half is that the test beside the comment had the truth the whole time. **Five things were met by an interpreted path and the record labels each one**: criterion 7 on `goal_contributions` and `users.email` rather than the brief's unobservable column; criterion 9's no-repeat proof on a static `categories?limit=5` because the only table over one page is `admin_audit_log`, which grows on every view of itself (the write-between-pages case a browser cannot stage is cited to `browse_repo_test.go:160`, which covers it); criteria 2 and 6 read through the Performance Timeline and `fetch(...).text()` rather than devtools' Network tab; and the stack brought up with `docker compose up -d --build` rather than a foreground `make dev`. **The walk also made one wrong claim of its own, caught in review and retracted in the file**: it said the brief's criterion 13 SQL omitted a `NOT NULL` column and so could not prove anything: running it shows `cannot execute INSERT in a read-only transaction`, because both of the role's guards fire in `ExecutorStart` and `NOT NULL` is checked in `ExecutorRun` — the brief was right. **The remaining gap is a deployment decision, not a missing capability:** production ships with it switched off — the product owner's decision on 2026-09-04 to ship dark: merged and deployed with `DATABASE_READONLY_URL` unset, so the live box shows the not-configured panel until an operator runs `deploy/PROVISION.md` section 10. Built everywhere, on nowhere real. **What it is:** two granted routes, `GET /admin/db/tables` and `GET /admin/db/tables/{table}?limit=&offset=`, and one screen at `/admin/database` (a fourth operator nav link) — a table list with row and column counts, then one page of one table. No filters, no sorting, no joins, no export and no SQL box; `psql` over SSH stays the tool for anything more (`deploy/README.md`'s Break-glass now starts here instead). **The guard is Postgres's, not this codebase's.** It reads through a *second* connection pool as `hearth_readonly`, a role granted `SELECT` on `public` and nothing else, with `default_transaction_read_only` and a 3-second `statement_timeout` set on the role itself. Only the timeout is *also* set on the pool (`readonly_pool.go`), as a connection runtime parameter, so that a box provisioned from an older `readonly-role.sql` is still bounded — the read-only setting is role-side only, and the pool's own contribution to the write guard is the check below rather than a second copy of that setting. `postgres.OpenReadOnly` runs a write-privilege check in `AfterConnect`, so it holds for *every* connection the pool ever opens rather than only the first, and the API refuses to boot when that check finds the role **can** write; `NewBrowseRepo` takes a `*ReadOnlyDB`, a distinct type, so the adapter cannot be built over the read-write pool by mistake. A table name is matched against `information_schema` through that same pool first — which makes it a privilege check as well as an existence check — and quoted with `pgx.Identifier` second, never concatenated. Redaction fails closed by column *type* (`bytea` and `bytea[]`) as well as by name (`_hash`, `_secret`, anything containing `password`, case-insensitively), and is emitted as a literal **inside the `SELECT` list**, so a secret column is never fetched at all: `«redacted»` on screen, and its bytes never leave Postgres. The type rule reads `udt_name` as well as `data_type`, because `data_type` reports a *category* rather than a type name for arrays (`ARRAY`) and for domains and extension types (`USER-DEFINED`) — six columns of this schema already do; the one shape it still cannot reach is a **domain** over `bytea`, which is stated as a limit rather than glossed, and is held by `browse_repo_test.go`'s schema sweep (added in the final fix wave — the test the design spec named as the one that would be easy to omit, and which the plan then omitted). `«null»` is a separate marker, because "you may not see this" and "there is nothing here" are different facts and a `[][]string` merges them. Paging is `ORDER BY` the primary key, `ctid` when a table has none — `OFFSET` without `ORDER BY` repeats and skips rows the moment anything writes between two pages, silently. The audit log needed no change at all: `auditAdmin` already records the path and the query string, which is where the table name and the offset live — which is also why paging goes through the URL rather than component state. **Two things beyond the parent spec §4 are worth knowing because both are invisible until long after the change that caused them.** `ALTER DEFAULT PRIVILEGES FOR ROLE hearth` is in the role script: without it, `GRANT SELECT ON ALL TABLES` covers only the tables that existed when it ran, and the next migration's table is missing from the browse with no error anywhere. And unconfigured, misconfigured and unavailable are **three** answers, not one: unset gives `503 DB_BROWSE_NOT_CONFIGURED` naming the variable (never a fallback to the read-write pool, and never an empty table list, which would read as "this database has no tables"); an unparseable DSN or one that can write **refuses the boot**; but a database that is merely unreachable **does not** — it answers `503 DB_BROWSE_UNAVAILABLE` from a stand-in browser that carries the boot failure so the log can say why. That last distinction is the restore-day case: the day a set variable meets a missing role is the day someone is rebuilding the box, and taking the household product down over an operator panel would invert the whole promise. **The infrastructure dependency the row always warned about is real**: `hearth_readonly` is cluster-level, so it is created by `deploy/readonly-role.sql` during provisioning rather than by a migration — and, for the same reason, it is in **no backup** (`backup.sh` dumps one database with `--no-privileges`), so a restore re-runs that script. It is idempotent, so "after every restore" is the whole instruction. See [ADR 5](adr/0005-platform-admin-authorization.md), amended 2026-09-04 for what this feature proved about its own narrowing |
...
| Personal API tokens for headless callers | ✅ | [ADR 7](adr/0007-personal-api-tokens.md), migration `00016`. `Authorization: Bearer hearth_…` resolves to the member's own Scope; Bearer wins and never falls back to the cookie; CSRF is skipped for a resolved token only; a token cannot mint, revoke, sign out or reach `/admin`; expiry required (90 days default, 365 max); membership update and removal revoke tokens beside sessions. `hearthctl token create|list|revoke` from a browser session, `hearthctl login --token` / `HEARTH_TOKEN` on the headless side. **Walked live 2026-09-08**: minted, listed (prefix only), signed in with it in a second config dir, `whoami` as the owner, a category written with no CSRF header, `token create` and a raw `POST /auth/tokens` through the token both refused (`SESSION_REQUIRED`), `/admin/flags` through the token 404, revoked from the browser session, next call exit 2. Three mutations (cookie fallback, CSRF skip removed, member removal no longer revoking tokens) each turned a test red. **Gap, closed 2026-09-23:** Settings' Access panel (the next row) now lists, creates and revokes a member's own tokens |
| Manage API tokens in Settings | ✅ | Settings' **Access panel** (`AccessPanel.tsx`) lists a member's own live API tokens — name, prefix, created, last used, expiry — with **Revoke** on their own rows only (in-page confirm, not `window.confirm`), and a **New token** dialog (`NewApiTokenModal.tsx`) offering 30 / 90 (default) / 365-day expiry that shows the raw `hearth_…` secret exactly once, Copy only, and drops it from memory on close. `ApiTokenList.tsx`, `useHouseholdAccess.ts`. Spec: `docs/superpowers/specs/2026-09-23-hearth-household-access-list-design.md` (decision 11). **Walked 2026-09-23 — 14 of 14**, in real Chromium with owner, partner and limited-member sessions: `docs/superpowers/plans/2026-09-23-hearth-household-access-list-m3-verification.md` |
| Household access list — every member's tokens and chats (no mockup) | ✅ | Not in the design at all: previously an owner could not answer "who and what can get into our household right now?" without a terminal, and could not see a partner's token or chat at all. New route `GET /api/v1/household/access`, **browser session only** (`requireCookieSession` — a leaked API token must never be able to enumerate every member's token prefixes and chat usernames). The handler reads the caller's role and picks the scope itself: an **owner** gets `AccessListService.ForHousehold` (every member's tokens and chats); a **limited member** gets `ForMember` (their own only) — the frontend never decides this. `telegramEnabled` is true only when a bot is configured **and** the household's `telegram_sign_in` flag is on; false hides the Linked chats group and withholds chat data entirely. `AccessPanel.tsx` renders both groups plus a one-line pointer to the pending-invites list already in Members (not repeated here, spec decision 2). Spec: `docs/superpowers/specs/2026-09-23-hearth-household-access-list-design.md`. **Walked 2026-09-23 — 14 of 14**, in real Chromium with owner, partner and limited-member sessions: `docs/superpowers/plans/2026-09-23-hearth-household-access-list-m3-verification.md` |
| Telegram chat commands — `/spend`, `/income`, `/balance`, `/recent`, `/help` | ✅ | [ADR 8](adr/0008-authorisation-at-each-channels-inbound-edge.md), spec `docs/superpowers/specs/2026-09-08-hearth-telegram-commands-design.md`. A fixed grammar with quoted names; amounts parsed in the account's currency with integer arithmetic (`domain.ParseAmount`, JPY 0 places, BHD 3); account defaults to the household's only cash account or is refused with the list; categories offered by kind; dated today, paid by the sender; the update id is the idempotency key so Telegram's redelivery replays. The guard — owner **and** money, chat resolved to a membership first — is the adapter's `Commander`, the channel's edge, not a service. Nineteen tests across domain, usecase and adapter, including the poller dispatching only with `WithCommands`. **Walked live on 2026-09-08** against a development bot (`@HearthOinkDevBot`) once the owner created one — the shared production token had made the dev poller lose every update to the other consumer (`terminated by other getUpdates request`). The owner sent `/balance`, `/spend 4.50 coffee` and `/recent` from their already-linked chat; the ledger holds `expense 450 coffee` with idempotency key `telegram-update-172668165`. Not walked by hand: the unlinked-chat refusal and the limited-member refusal (both need a second Telegram account; covered by adapter tests) |
| Telegram free text — "spent 84.50 on groceries", read by a language model, written on `/yes` | ✅ | Stage 5b of the same spec. `usecase.IntentParser` port; `adapter/anthropic` implements it with `claude-opus-5` at effort low, one strict `log_transaction` tool, the household's names in the prompt, and a fail-closed read (no tool call, a refusal, or an unknown kind is "none"). **Never a write on its own**: the Commander shows the reading back and holds it five minutes; `/yes` writes it keyed by the sentence's update id, `/no` discards, a stale or second `/yes` writes nothing. The guard runs before the parser, so a stranger's sentence never reaches the API. Off unless `ANTHROPIC_API_KEY` is set — the bot says "commands only". Tested against a fake Messages API that pins the request shape, plus the confirm flow end to end with mutations for "write before /yes" and "/yes after expiry". **Now OpenRouter only:** an open-weight model through `adapter/openrouter` (plain `net/http`, forced tool call, `OPENROUTER_API_KEY` + up to three `OPENROUTER_MODEL` ids tried in order), with prompt, schema and the fail-closed reader in `adapter/intent`; every parser call capped at 30 s by the Commander. Built first on Claude; the owner's first call was refused for lack of credits, they chose free models, and the Claude adapter was removed (04c4da9 holds it). **Walked live on 2026-09-08** from the owner's chat on `@HearthOinkDevBot` with the three-model free list: "spent 12.30 on lunch" read back and written on `/yes` — the ledger holds `expense 1230 lunch` keyed `telegram-update-172668172`, once; "hello there" answered "could not read that as an expense or income" and wrote nothing. Two earlier attempts failed on a single model's `429` and on a four-id list (OpenRouter caps it at three), both now refused or routed around at boot. **Known, accepted:** no per-chat ceiling on parser calls for an authorised owner yet (a stranger's or limited member's sentence never reaches the API, and is not even answered); on free models that costs nothing, so it waits for a paid one |
| Daily digest — the bot speaks first: bills due within three days, budget lines past 80% | ✅ | Stage 6, spec `docs/superpowers/specs/2026-09-08-hearth-nudges-design.md`, [ADR 9](adr/0009-scheduled-work-runs-inside-the-api.md). Rules, not a model; one message per linked owner-with-Money per household per day; autopay bills excluded; nothing to say means no message. At-most-once by a claimed `nudge_deliveries` row (migration `00017`), released on a failed send. Scheduler in-process on a 15-minute tick; `NUDGES_AT` + `NUDGES_TIMEZONE`, both or neither, refused without Telegram. The recipients query is ADR 8's outbound half (owner **and** Money). **Walked live 2026-09-08**: a bill due in two days, `nudge sent` in the log (Telegram accepted the send), one ledger row, and a restart that sent nothing; the rendered message and a live `/nudges off` → `on` are still to be seen by the owner. **Known gap:** one time zone for the whole install; the column belongs on `households` when a second zone signs up |
| `/nudges on\|off` — stop or resume the digest for this chat | ✅ | Same guard as `/spend`; stored on `telegram_accounts.nudges_enabled` (per phone, not per household). Says "not configured on this install" when the digest is off rather than saving a choice nothing reads. Tested: the toggle, usage on anything else, a limited member never reaches it; the postgres recipients test proves an opted-out owner is skipped |
...
**none, now.** All four are settled: the audit screen was built, walked and
then descoped the same day (its row in section 9 says why), households and
metrics was built later that same day, and the outbound message inspector
and the read-only database browse were both built 2026-09-04. That was a
preference, not a dependency, and it is worth naming as one — see "what this
costs" below. The order *within* each group is still dependency.

**First — finish the operator surface (section 9). Done, 2026-09-04.**
Items 1–4 below are all struck through and section 9 has no ⬜ left. The
numbering is kept so earlier references to "item 2" and "item 4" still point
at the same things. One piece of unfinished business survives the group and
is deliberately *not* an item of its own: the flags screen's named 🟡 gap —
there is still no control to create a per-household override, and the
household list a picker would need has existed since 2026-09-02. It is a
small addition to a built screen, not a slice.

**So the next work is item 6, Marriage's remaining ⬜ — and Agreements is
the interesting one:** it is append-only and versioned rather than CRUD,
...
**Item 6 is done, 2026-09-06 — built, reviewed and walked.**
...
in 22 numbered decisions, eighteen tasks later the four tables, the
service, the seven routes and both screens were built and reviewed
(`915edba`), and Task 18's own fifteen-criterion browser walk then ran and
passed, 15 of 15, on 2026-09-06 — every row above now reads ✅. Item 6 will
...
4. ~~**Read-only database browse**~~ (§4) — **built 2026-09-04**, to its own
   spec, `docs/superpowers/specs/2026-09-04-hearth-database-browse-design.md`,
   which expands §4 of the admin-surface spec. Last of the four despite being
   the one originally asked for, and the sequencing was right on both counts:
   it was the only one with an infrastructure dependency — `hearth_readonly`
   is cluster-level, so it is provisioned rather than migrated, and
   `deploy/PROVISION.md` gained a section for it — and it had much the
   largest security surface. It was also the piece the other three de-risked:
   each of them exercised the re-auth grant and the audit log under real use
   before the surface that can read every household's finances arrived, and
   this one needed no audit change at all as a result. **Section 9's row is
   ✅**: its fifteen-criterion browser walk ran the same day and passed 15 of
   15 (`docs/superpowers/plans/2026-09-04-hearth-database-browse-verification.md`),
   which was the one of that row's two stated reasons for 🟡 that a walk could
   close. The other stands and is not a verification gap: production ships
   with the feature switched off by the owner's decision, so it is verified
   everywhere and running nowhere real until an operator runs
   `deploy/PROVISION.md` section 10. Numbering kept, as for items 1–3.

**Then — the household product**, dependencies as before. **As of 2026-09-04
this group is no longer "then": it is next**, and item 6 is where it starts.

5. **Money's remaining 7 ⬜** — 25 of 35 rows are ✅; what is left is the tail,
   not the slice
6. ~~**Marriage's remaining ⬜**~~ — **built, reviewed and walked, 2026-09-06**
   (`915edba`, eighteen tasks, walk in
   `docs/superpowers/plans/2026-09-05-hearth-agreements-verification.md`,
   15 of 15): Agreements' four tables, service, seven routes and both
   screens, to its own spec. Of Marriage's now twenty rows, 18 are ✅. The
   2 remaining ⬜ (Vision's marriage-duration row, "Popular starting
...
household member can see any of it. Prioritising it meant the thing being sold
stood still while the thing that runs it improved. The case for doing it
anyway was real and is why the reprioritisation was made: the install is
