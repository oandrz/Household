...
**Result: 15 of 15 pass.** No product defect needed a code fix. Two criteria
(2 and 10) were reordered relative to the brief's own numbering for a
dependency reason named inline below; criterion 10 was walked by two
different paths (an interpreted propose-time one and a literal sign-time
one) because the brief's own wording anticipated either could be the one the
running code actually takes, and both are recorded rather than only the one
that happened to fire first. One genuine, reproducible finding surfaced
during criterion 14 — not a Hearth code defect, confirmed by an isolated
zero-app-code repro — and is recorded under "Findings, not defects" below,
in the same shape Vision's own walk used for its own non-defect findings.
...
**Score: 15 of 15 pass.** No product defect needed a code fix anywhere in
this walk. `git status --short api web` reads empty at the end of this
task — the only files this walk touches are the two it was told it may
touch (this record and `docs/FEATURE_TRACKER.md`/`docs/LEARNING.md`) plus
the screenshots directory; `make lint && make test` (Step 8) was re-run
anyway, on the same tree, and both are green (see that section below).
...
literal path, or are code-level observations that could not be turned into
an actual reproduced defect — recorded rather than silently passed over,
the standard Vision's own walk set for this section.
...
2. **A background refetch reaching a still-open Propose modal could, in
   principle, silently change what `previousBody` compares against —
   investigated, not reproduced.** `ProposeAgreementModal.tsx`'s own
   `bodyOf(targetId)` reads the live `sections` PROP at send time
   (`previousBody: mode === "add" ? "" : bodyOf(targetId)`), not a value
   snapshotted when the editor opened or when the target was last chosen —
   and `sections` is exactly the prop that changes on a background
   refetch, since it flows straight from `AgreementsPage`'s own
   `doc.sections`. If that refetch landed while a household member's own
   modal sat open past `staleTime` (30s) and their tab regained real OS
   focus, `previousBody` would silently match whatever the document now
   says rather than what the editor was actually opened against — the
   opposite of the guarantee the same file's own comment states ("wording
   the proposer never saw must never be what gets compared"). This walk
   tried, deliberately, to reproduce it live: waiting 32+ seconds past
   `staleTime` and calling `page.bringToFront()` on the held-open tab,
   dispatching synthetic `focus`/`visibilitychange` events directly, and
   opening a same-context decoy tab to force a genuine tab-switch — none
   of it caused Playwright's automation-controlled pages to report
   `document.hasFocus() === false` at any point, so the
   `refetchOnWindowFocus` path this concern depends on was never actually
   exercised in a real browser here, only reasoned about from the code.
   Per this project's own standard (verify in the real environment, not
   from a stub or a read of the source alone), this is recorded as an
   **open question for a future walk with a real, unautomated browser to
   settle**, not as a confirmed defect and not as a code change made on the
   strength of code-reading alone.

---

## Step 4: screenshots compared by hash, not by eye

`shasum -a 256` over all 45 PNGs in
`docs/superpowers/plans/2026-09-05-hearth-agreements-screenshots/` found
**44 distinct hashes and exactly one repeated pair**:
`04b-after-starter-set.png` and `05c-after-withdraw.png` are byte-identical.
...
No code changed during this walk, so this is a confirmation run on the
tree the walk started from, not a regression check against a fix:

```
make lint   # arch-lint, tsc, eslint, go vet — all green
make test   # api: 13 packages ok; web: 88 files / 826 tests passed
```
...
- **Agreements**: three live, numbered continuously — `01` Money ("No solo
  spend over $400, and we review it every January." — the wording that
  survived criterion 10's whole edit-race sequence), `02` Conflict ("When
  we disagree about the kids, we step outside the room before raising our
...
- **Proposals**: zero open. Every proposal this walk created was either
  withdrawn or agreed by the end of the criterion (or the off-script pass)
  that created it — none left dangling.
- **Version history**: eight-plus entries accumulated across the walk
  (every edit and the one remove/restore pair this walk made), all real,
  none touched directly in the database.
- **No SQL was run against any Agreements table during this walk** — every
  ...
  the deliberately-engineered conflicts in criterion 10, which needed three
  real signed-in sessions rather than any shortcut.
- `hearth_hearth-pgdata` was dropped once, at the very start (see the
  fresh-seed note above), and not touched again.

---

## Screenshots: 45 files, 44 distinct hashes
