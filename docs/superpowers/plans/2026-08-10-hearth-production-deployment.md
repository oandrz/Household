...
- `docs/SYSTEM_DESIGN.md` §1 — flip **deployed**, not **built**. Tasks 1–6 already moved that document from "not built" to "built, not yet deployed": the heading reads "The production topology — built, not yet deployed", the admin node reads "built, never run on a box", and the scope statement at the top of the file says the deployment is written but nothing runs. What this step changes is only the second half of each of those — the topology becomes a description of something running, the admin node loses "never run on a box", and the scope statement says where it runs. **Do not reintroduce any "NOT BUILT" or "does not exist" wording while editing around it.** The dashes on the admin edge stay: they mean occasional rather than request-path traffic, exactly as on `Caddy -.-> LE` and `PG -.-> Backup`, and never meant "missing"
- `docs/SYSTEM_DESIGN.md` §8 — Hosting, TLS, Backups and Production administration rows all change state. Hosting and Production administration go from "built, not deployed" to deployed; Backups goes from "written and rehearsed, not scheduled" to running on a schedule with the escrow exercised (criterion 12), which is the half of that row this walk actually closes
