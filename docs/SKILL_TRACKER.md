...
| **`hearth-cli`** | A task says to put data into Hearth, script it, automate it, or read what a household holds without a browser. | The product now has a command-line client (`hearthctl`, ADR 6) so an agent can drive it — and an agent that does not know the loop (`whoami`, resolve ids, insert, read back) or the one rule that matters (never retry a password: five failures lock the household) is a liability with a shell. This skill is the manual an agent loads before touching the CLI. |
